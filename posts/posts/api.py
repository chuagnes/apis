...
def post_delete(id):
    ...
    message = "Sucessfully deleted post with id {}".format(id)
    data = json.dumps({"message":message})
    return Response(data, 200, mimetype="application/json")

#API endpt to add posts 
...
#API endpt to update single post 
...
def post_edit(id):
    ...
    #edit post 
    post = models.Post(title=request.form["title"], body=request.form["content"])
    session.add(post)
    session.commit()
